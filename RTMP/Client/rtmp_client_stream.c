...
// ./rtmp_client_stream *.mp4 rtmp://192.17.20.207/live
// ./rtmp_client_stream *.flv rtmp://127.0.0.1/live
// ./rtmp_client_stream rtmp://192.17.20.207/live *.flv
int main(int argc, char **argv) {
    AVOutputFormat *ofmt = NULL;
    AVFormatContext *ifmt_ctx = NULL, *ofmt_ctx = NULL;
    AVPacket pkt;
    char in_filename[1024] = {0}, out_filename[1024] = {0};
    int ret, i;
    int stream_index = 0;
    int *stream_mapping = NULL;
    int stream_mapping_size = 0;
    bool push_stream = false;
    char ofmt_name[512] = {0};
    ...
    if (strstr(out_filename, "rtmp://") != NULL) {
        push_stream = true;
        sprintf(ofmt_name, "flv");
    }
    else if (strstr(out_filename, "udp://") != NULL) {
        push_stream = true;
        sprintf(ofmt_name, "mpegts");
    }
    else {
        push_stream = false;
    }
